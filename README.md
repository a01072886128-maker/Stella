...
- **현상**: 히스토리 저장 시, 사용자가 입력한 텍스트와 이미지 파일을 하나의 요청으로 동시에 서버에 전송하지 못함.  
- **세부 내용**: 텍스트만 전송은 가능했으나, JSON 객체에 텍스트와 이미지를 함께 담아 전송했을 때 서버가 요청을 전혀 인식하지 못하는 문제가 발생.  
- **원인 분석**:  
...
- 향후에는 **멀티파트(Multipart) 전송 방식**을 적용하여 텍스트와 파일을 효율적으로 처리하는 방안을 고려할 예정.  


- **문제2: GPT4O API 모델의 한계**  
  - OpenAI GPT API를 호출했으나, 일반 한국어 대화에는 대응했지만 **여자어 특유의 맥락/이중 의미**를 제대로 번역하지 못함.  
  - 특히 한국어 특성(존칭, 뉘앙스, 이모티콘, 선톡 여부 등)에 대한 학습 데이터가 부족하여 원하는 품질이 나오지 않음.  

- **해결: 맞춤형 CSV 데이터 학습 적용**  
  - 이를 보완하기 위해 **여자어 대화 사례를 직접 수집 → CSV 파일 형태로 가공 → 감정 레벨과 상황별 대응 라벨링** 진행.  
  - GPT API에 이 CSV 데이터를 Few-shot Prompting 방식으로 제공하여,  
    → **입력된 문장이 데이터셋에 없더라도 유사한 맥락을 추론하여 답변**을 생성할 수 있도록 설계.  
  

- **한계 & 향후 개선 방향**  
  - 데이터 양이 아직 부족해 결과가 들쭉날쭉할 때가 있음  
  - 데이터 가공(라벨링, 감정 점수화) 과정에 시간이 많이 소요됨  
  - 추후 연구로 확장 시, **대규모 한국어/여자어 특화 데이터셋**을 확보하여 성능 개선 계획
